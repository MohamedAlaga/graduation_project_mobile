    ...
    if (allQuestionsAnswered) {
      for (int i = 0; i < widget.quizQuestions.length; i++) {
        if (selectedAnswers[i] == 0) {
          widget.quizQuestions[i].userSelectedAnswerId =
              widget.quizQuestions[i].answerOneId;
        } else if (selectedAnswers[i] == 1) {
          widget.quizQuestions[i].userSelectedAnswerId =
              widget.quizQuestions[i].answerTwoId;
        } else if (selectedAnswers[i] == 2) {
          widget.quizQuestions[i].userSelectedAnswerId =
              widget.quizQuestions[i].answerThreeId;
        }
        print(
            'question : ${widget.quizQuestions[i].questionText} , answer is :${widget.quizQuestions[i].userSelectedAnswerId} ');
      }
      var prefs = await SharedPreferences.getInstance();
      String token = prefs.getString('token').toString();
      var y = await saveTest(widget.quizQuestions, token);
      var x = await getQuizAnswer(token);
      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => Page18(grade: percentage),), (route) => false);
      // Navigator.push(
      //   context,
      //   MaterialPageRoute(
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            title: const Center(
                child: Text('الامتحان غير كامل', style: Styles.style20r)),
            content: const Text(
                      ...
                      Column(
                        ...
                        children: List.generate(3, (answerIndex) {
                          ...
                          switch (answerIndex) {
                            case 0:
                              answerText =
                                  widget.quizQuestions[index].answerOneText ??
                                      '';
                              break;
                            case 1:
                              answerText =
                                  widget.quizQuestions[index].answerTwoText ??
                                      '';
                              break;
                            case 2:
                              answerText =
                                  widget.quizQuestions[index].answerThreeText ??
                                      '';
                              break;
                            default:
                              answerText = '';
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      color:
                                          getColorForAnswer(index, answerIndex),
